...
let
  ...
  wordSeparatorSubstitution = "#WORDCHARS='\${WORDCHARS:s@/@}'";
in
{
  # include packages that are just shell scripts.
  home.packages = [
    (pkgs.writeShellScriptBin "ggptemacs" ''
  ...
  programs.zsh = {
    ...
    '' else "") + ''
      ...
      ggptemacs() {
          # Set API keys if not already present
          if [[ -z "''${OPENAI_API_KEY}" || -z "''${ANTHROPIC_API_KEY}" ]]; then
              echo "Setting OPENAI_API_KEY and ANTHROPIC_API_KEY using passage" >&2
              export OPENAI_API_KEY="$(passage openai-api-key-redbow)"
              export ANTHROPIC_API_KEY="$(passage console.anthropic.com/api-redbow)"
      ...
      #WORDCHARS=.
      #WORDCHARS='*?_[]~=&;!#$%^(){}'
      #WORDCHARS=${wordSeparatorSubstitution}
